...
if (isset($_GET['user'])) {
    if ($_GET['user'] == 'update') {
        ...
        header('location: /src/pages/secure/admin/user.php' . $params);
    }
    ...
    if ($_GET['user'] == 'delete') {
        ...
        if ($user['administrator']) {
            ...
            header('location: /src/pages/secure/admin/');
            return false;
        }
...
function updateProfile($req)
{
    ...
    if (isset($data['invalid'])) {
        $_SESSION['errors'] = $data['invalid'];
        $params = '?' . http_build_query($req);
        header('location: /src/pages/secure/user/profile.php' . $params);
        } else {
        $user = user(); 
        $data['id'] = $user['id'];
...
function changePassword($req)
{
    ...
    if (isset($data['invalid'])) {
        $_SESSION['errors'] = $data['invalid'];
        $params = '?' . http_build_query($req);
        header('location: /src/pages/secure/user/password.php' . $params);
    } else {
        $data['id'] = userId();
        $success = updatePassword($data);
        if ($success) {
            $_SESSION['success'] = 'Password successfully changed!';
            header('location: /src/pages/secure//user/password.php');
        }
    }
}
